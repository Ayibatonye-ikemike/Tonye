class budget():
  ...
  def _init_(self,food,clothing,entertainment):
  ...
  def balance(db):
    for food,clothing,entertainment,bal in db.items():
      print(food,clothing,entertainment)
      
  def transfer (db,option1,amount,option2):
    value1 = db[option1]
    value2 = db[option2]
    
    db[option1] = int(value)-amount
    db[option2] = int(value)+amount
    return db
...
def select_budget(budget_title):
    print("\n=== ***select Budget**** ===\n")

    budget_title = int(input("\nPress (1) food\nPress (2) clothing\nPress (3) enteertainment\n"))
    try:
        amount = int(input("Enter your budget amount \n$"))
    except:
        print('\nInvalid input')
        select_budget()
select_budget = select_budget (budget_title, amount)
...
def debit():
    print("=== ****Withdraw from a selected budget**** ===\n")
